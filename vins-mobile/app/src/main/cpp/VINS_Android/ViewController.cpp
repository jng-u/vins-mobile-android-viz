...
std::vector<float> ViewController::getCorrectPC() {
    std::vector<float> ret;
//    ret.resize(vins.correct_point_cloud.size() * 3);
//    for(unsigned int i=0; i<vins.correct_point_cloud.size(); i++) {
//        ret.push_back(vins.correct_point_cloud[i][0]);
//        ret.push_back(vins.correct_point_cloud[i][1]);
//        ret.push_back(vins.correct_point_cloud[i][2]);
//    }

    for(auto fpi : vins.f_manager.feature) {
        for(auto ft : fpi.feature_per_frame) {
            ret.push_back(ft.point[0]);
            ret.push_back(ft.point[1]);
            ret.push_back(ft.point[2]);
        }
    }
    return ret;
}

std::vector<float> ViewController::getPath() {
    std::vector<float> ret;
     for(unsigned int i=0; i<vins.drawresult.pose.size(); i++) {
         ret.push_back(vins.drawresult.pose[i][0]);
         ret.push_back(vins.drawresult.pose[i][1]);
         ret.push_back(vins.drawresult.pose[i][2]);
     }
//    for(unsigned int i=0; i<keyframe_database.refine_path.size(); i++) {
//        ret.push_back(keyframe_database.refine_path[i][0]);
//        ret.push_back(keyframe_database.refine_path[i][1]);
//        ret.push_back(keyframe_database.refine_path[i][2]);
//    }
    return ret;
}